...
import { Plus, Trash2, FileDown, AlertTriangle, Search, Pencil } from "lucide-react";
import type { PurchaseSaleItem } from "@workspace/api-client-react";


export function Sales({ onNavigate, canCreate, canEdit, canExport }: { onNavigate: (v: CrmView, id?: number) => void; canCreate: boolean; canEdit: boolean; canExport: boolean }) {
  ...
  const availableFor = (productId: number) => {
    ...
    return stockLevels?.find(l => l.productId === productId && l.warehouseId === warehouseId)?.quantity ?? 0;
  };

  const handleQuantityChange = (i: number, productId: number, productName: string, requested: number) => {
    const avail = productId ? availableFor(productId) : undefined;
    if (avail !== undefined && requested > avail) {
      alert(`Only ${avail} ${productName || "unit(s)"} available in stock. Quantity has been set to ${avail}.`);
      updateItem(i, { quantity: avail });
      return;
    }
    updateItem(i, { quantity: requested });
  };

  const searchTerm = search.trim().toLowerCase();
  const filteredSales = (sales || []).filter(s =>
    !searchTerm ||
    s.invoiceNumber.toLowerCase().includes(searchTerm) ||
    (customers?.find(c => c.id === s.customerId)?.companyName || "walk-in").toLowerCase().includes(searchTerm)
  );
  const pagedSales = paginate(filteredSales, page);
  ...
  const handleSave = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    createMutation.mutate({
      ...
      onError: (err) => setFormError((err as Error).message || "Could not save sale"),
    });
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-heading font-bold text-[#093C71]">Sales</h1>
          <p className="text-sm font-medium text-slate-500 mt-1">Recording a sale automatically decreases stock; insufficient stock is blocked.</p>
        </div>
        {canCreate && !showForm && <button onClick={() => setShowForm(true)} className={btnPrimaryClass}><Plus size={16} /> New Sale</button>}
      </div>
      ...
      {showForm && (
        <form onSubmit={handleSave} className={`${cardClass} p-6 space-y-6 border-t-4 border-t-[#093C71]`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-5">
            <div><label className={labelClass}>Sale Date *</label><input required type="date" name="saleDate" defaultValue={new Date().toISOString().slice(0, 10)} className={inputClass} /></div>
            <div>
              <label className={labelClass}>Customer</label>
              <select name="customerId" className={inputClass}><option value="">Walk-in / none</option>{customers?.map(c => <option key={c.id} value={c.id}>{c.companyName}</option>)}</select>
            </div>
            <div>
              <label className={labelClass}>Warehouse *</label>
              <select required name="warehouseId" value={warehouseId} onChange={e => setWarehouseId(Number(e.target.value))} className={inputClass}>
                  ...
                  {items.map((it, i) => {
                    ...
                    return (
                      <tr key={i}>
                        ...
                        <td className="px-3 py-2"><input ref={el => { qtyRefs.current[i] = el; }} type="number" min="0" step="1" value={it.quantity === 0 ? "" : it.quantity} onChange={e => handleQuantityChange(i, it.productId, it.productName, Number(e.target.value))} className={`${inputClass} text-right`} /></td>
          ...
          <div className="flex justify-end gap-3 pt-3 border-t border-slate-200">
            ...
            <button type="submit" disabled={createMutation.isPending} className={btnPrimaryClass}>Save Sale</button>
          </div>
        </form>
      )}
      ...
      <div className={cardClass}>
        <table className="w-full text-sm text-left">
          <thead className="bg-slate-50 border-b border-slate-200 text-xs uppercase tracking-wider text-slate-500 font-bold">
            <tr><th className="px-5 py-4">Invoice No</th><th className="px-5 py-4">Date</th><th className="px-5 py-4">Customer</th><th className="px-5 py-4">Status</th><th className="px-5 py-4 text-right">Total</th><th className="px-5 py-4 w-10"></th></tr>
          </thead>
          <tbody className="divide-y divide-slate-100">
            {isLoading ? <LoadingRow colSpan={6} />
            : pagedSales.length ? pagedSales.map(s => (
              <tr key={s.id} className="hover:bg-slate-50">
                <td className="px-5 py-4 font-bold text-[#093C71]">{s.invoiceNumber}</td>
                <td className="px-5 py-4 text-slate-600">{formatDate(s.saleDate)}</td>
                <td className="px-5 py-4 text-slate-600">{customers?.find(c => c.id === s.customerId)?.companyName || "Walk-in"}</td>
                <td className="px-5 py-4"><span className="px-2 py-0.5 rounded-full text-[11px] font-bold bg-green-100 text-green-700">{s.status}</span></td>
                <td className="px-5 py-4 text-right font-bold text-slate-800">₹{s.totalAmount.toLocaleString("en-IN")}</td>
                <td className="px-5 py-4 text-right flex items-center justify-end gap-1">
                  {canEdit && <button onClick={() => onNavigate('sale-form', s.id)} className="p-2 text-slate-400 hover:text-[#093C71]" title="Edit"><Pencil size={16} /></button>}
                  {canExport && <button onClick={() => openDocPdf(`/api/crm/sales/${s.id}/pdf`)} className="p-2 text-slate-400 hover:text-[#093C71]"><FileDown size={16} /></button>}
                </td>
              </tr>
            )) : sales?.length ? <tr><td colSpan={6} className="px-5 py-8 text-center text-slate-500">No records found</td></tr>
            : <EmptyRow colSpan={6} actionLabel={canCreate ? "New Sale" : undefined} onAction={canCreate ? () => setShowForm(true) : undefined} />}
          </tbody>
        </table>
        <Pagination page={page} onPageChange={setPage} totalItems={filteredSales.length} />
      </div>
    </div>
  );
}
